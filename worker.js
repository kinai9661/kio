/**
 * KIO Gateway v5.0 - Split Canvas UI
 * Left sidebar (accordion) + Right large preview + Bottom history strip
 */
...
:root{
  ...
  --text:#e2e2ef;--text2:#6e6e85;--text3:#4a4a5e;
...
.logo-ver{font-size:.6rem;color:var(--text3);-webkit-text-fill-color:var(--text3);margin-left:2px;}
...
.api-tab{flex:1;padding:7px 4px;font-size:.66rem;font-weight:600;color:var(--text2);cursor:pointer;text-align:center;border-bottom:2px solid transparent;transition:all .15s;}
...
.copy-btn{
  ...
  color:var(--text2);cursor:pointer;float:right;margin-bottom:4px;
}
...
.empty-p{font-size:.75rem;max-width:220px;line-height:1.6;}
...
.preview-prompt{
  ...
  background:rgba(12,12,16,.8);border:1px solid var(--border);
  border-radius:8px;padding:7px 11px;
  font-size:.72rem;color:var(--text2);line-height:1.5;
  backdrop-filter:blur(6px);max-height:72px;overflow:hidden;
}
.act-btn{
...
.hist-strip{
  ...
  padding:0 14px;gap:10px;overflow-x:auto;
  scrollbar-width:thin;scrollbar-color:var(--border) transparent;
}
...
@media(max-width:720px){
  ...
  .canvas-preview{min-height:300px;padding:14px;}
...
<div class="canvas">
  <div class="canvas-preview" id="canvasPreview">
    ...
    <div class="empty-state" id="emptyState">
      ...
      <div class="empty-h" id="lbl-empty-h">No image generated</div>
    ...
    <div class="preview-wrap" id="previewWrap">
      ...
      <img class="preview-img" id="previewImg" src="" alt="" onclick="openLb(this.src)">
      <div class="preview-actions" id="previewActions">
        <a class="act-btn primary" id="dlBtn" href="#" download="kio.png">&#11015;&#65039; Download</a>
        <button class="act-btn ghost" id="cpUrlBtn">&#128203; Copy URL</button>
        <button class="act-btn ghost" onclick="openLb(document.getElementById('previewImg').src)">&#128269; Zoom</button>
      </div>
    </div>
  </div>
...
var T={
  en:{
    'lbl-prompt':'PROMPT','lbl-settings':'SETTINGS','lbl-apikey':'API KEY',
    'lbl-debug':'API DEBUG','lbl-genbtn':'&#10024; Generate',
...
function applyLang(){
  var ids=['lbl-prompt','lbl-settings','lbl-apikey','lbl-debug','lbl-genbtn',
    'lbl-model','lbl-size','lbl-apikey-sub','lbl-empty-h','lbl-empty-p','lbl-history-empty',
    'progLbl','tab-req','tab-res','tab-poll'];
  ...
  document.getElementById('lbl-genbtn').innerHTML=tr('lbl-genbtn');
...
document.addEventListener('DOMContentLoaded',function(){
  ...
  applyLang();
  renderHist();
  if(hist.length>0){showPreview(hist[0].src,hist[0].prompt);}
});
...
function renderHist(){
  ...
  if(hist.length===0){strip.innerHTML='<div class="hist-empty" id="lbl-history-empty">'+tr('lbl-history-empty')+'</div>';return;}
  strip.innerHTML='';
  hist.forEach(function(item,i){
    var th=document.createElement('div');
    th.className='hist-thumb'+(i===activeHistIdx?' active':'');
    var img=document.createElement('img');img.src=item.src;img.alt='';
    var ov=document.createElement('div');ov.className='hist-ov';ov.textContent='&#128269;';
    th.appendChild(img);th.appendChild(ov);
    th.onclick=(function(idx,s,p){return function(){
      activeHistIdx=idx;showPreview(s,p);renderHist();
    };})(i,item.src,item.prompt);
    strip.appendChild(th);
  });
}

/* ===== Preview ===== */
function showPreview(src,prompt){
  ...
  cpBtn.onclick=function(){navigator.clipboard.writeText(src);cpBtn.innerHTML=tr('btn-copied');setTimeout(function(){cpBtn.innerHTML=tr('btn-copy-url');},2000);};
}
...
function setLoad(on){
  ...
  if(on)startProg(tr('sending'));else endProg();
}
async function generate(){
  ...
  var reqBody={prompt:prompt,model:model,size:selectedSize,n:1};
  ...
  try{
    ...
    if(!item)throw new Error(tr('err-no-img'));
    var imgSrc=item.url||(item.b64_json?'data:image/png;base64,'+item.b64_json:null);
    if(!imgSrc){showStatus('error',tr('err-no-url'));setLoad(false);return;}
    showPreview(imgSrc,prompt);
    saveHist(imgSrc,prompt);
    showStatus('success',tr('ok-gen'));
  }catch(err){
  ...
  async fetch(request, env, ctx) {
    ...
    try {
      ...
      if (request.method === 'GET' && url.pathname === '/health')
        return json({ status: 'ok', version: '5.0.0', ui: 'split-canvas', features: ['custom-api-key','i18n','1K-2K-4K'] });
      ...
      if (request.method === 'POST' &&
        ...
        const requestInfo = { prompt, model, n, size, endpoint: SUPABASE_URL + '/openai-compatible',
